...
/**
 * Undergraduate profile panel.
 *  - FlatLaf styling
 *  - Picture / phone editing
 *  - Secure password‑change workflow
 */
public class ProfilePanel extends JPanel {

    private final String userId;
    private String name, hashedPassword, email, phone, profilePicPath, role;
    ...
    private void initUI() {
        ...
        card.add(labelPair("User ID:", userId, y += gap, labelFont, valueFont));
    ...
    private JPanel labelPair(String label, String value, int y, Font labelFont, Font valueFont) {
        ...
        l.setFont(labelFont);
        l.setBounds(10, 0, 100, 25);
        p.add(l);

        JLabel v = new JLabel(value);
        v.setFont(valueFont);
        v.setForeground(Color.DARK_GRAY);
        v.setBounds(120, 0, 210, 25);
        p.add(v);
        return p;
    }
    ...
    private void handlePasswordChange() {
        ...
        try (Connection con = dbconnection.getConnection()) {
            ...
            String currentHash = Admin.encryption.hashpassword(currentPwd);
            String sql = "SELECT user_password FROM user WHERE user_id=?";
            PreparedStatement chk = con.prepareStatement(sql);
            ...
            String newHash = Admin.encryption.hashpassword(newPwd);
