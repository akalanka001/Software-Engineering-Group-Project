...
public class LecturerVideoPanel extends JPanel {
    ...
    private JLabel modulesCountLabel; // “n modules” indicator
    private static final long MAX_VIDEO_SIZE = 5L * 1024 * 1024 * 1024; // 5 GB
    ...
    private void initUI() {
        ...
        courseComboBox.addActionListener(e -> {
            reloadModules();
            String id = extractCourseId();
            if (id != null) updateModulesCountLabel(id);
        });
    ...
    // ---------------------------------------------------------------------
    private void updateModulesCountLabel(String courseId) {
        String sql = "SELECT modules FROM course WHERE course_id = ?";
        try (Connection con = dbconnection.getConnection();
             PreparedStatement pst = con.prepareStatement(sql)) {
            ...
            System.err.println("updateModulesCountLabel: " + e.getMessage());
        }
    }
    ...
    private void reloadModules() {
        ...
        updateModulesCountLabel(courseId);
    ...
    private void addModule() {
        ...
        // --- Limit enforcement ---
        int limit = getModuleLimit(courseId);
        int current = getCurrentModuleCount(courseId);
        if (limit > 0 && current >= limit) {
            showError("This course already has its maximum number of modules (" + limit + ").");
            return;
        }
        ...
        browse.addActionListener(e -> {
            JFileChooser fc = new JFileChooser(new File(System.getProperty("user.home"), "Videos"));
            fc.setFileSelectionMode(JFileChooser.FILES_ONLY);
            fc.setFileHidingEnabled(true);
            fc.setAcceptAllFileFilterUsed(false);
            fc.addChoosableFileFilter(new FileNameExtensionFilter("MP4 Video Files (*.mp4)", "mp4"));
            ...
            if (res == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    if (!name.endsWith(".mp4")) {
                        showError("Invalid file type! Please select a .mp4 video.");
                        return;
                    }
                    String mimeType = Files.probeContentType(selectedFile.toPath());
                    if (mimeType == null || !mimeType.equals("video/mp4")) {
                        showError("Selected file is not a valid MP4 video.");
                        return;
                    }
                    if (!validateVideoSize(selectedFile)) return;
        ...
        if (title.isEmpty() || path.isEmpty()) {
            showError("Title and video are required.");
            return;
        }
    ...
    private boolean validateVideoSize(File file) {
        ...
        if (size > MAX_VIDEO_SIZE) {
            ...
            showError(String.format("Selected video is %.2f GB. Maximum allowed is 5 GB.", gb));
            return false;
        }
        return true;
    }
    ...
    // ---------------------------------------------------------------------
    private int getModuleLimit(String courseId) {
        String sql = "SELECT modules FROM course WHERE course_id=?";
        try (Connection con = dbconnection.getConnection();
             PreparedStatement pst = con.prepareStatement(sql)) {
            pst.setString(1, courseId);
            ...
            System.err.println("getModuleLimit: " + e.getMessage());
        }
        return 0;
    }

    private int getCurrentModuleCount(String courseId) {
        ...
        try (Connection con = dbconnection.getConnection();
            ...
            System.err.println("getCurrentModuleCount: " + e.getMessage());
        }
        return 0;
    }
    ...
    private boolean verifyLecturerPassword(String pass) {
        ...
        try (Connection con = dbconnection.getConnection();
            ...
            showError("Password verification error: " + e.getMessage());
        }
        return false;
    }

    private void notifyStudents(String courseId, String message) {
        String select = "SELECT DISTINCT ug_id FROM enrollment WHERE course_id=? AND approval_status='approved'";
        ...
        try (Connection con = dbconnection.getConnection();
            ...
            showError("Notification failure: " + e.getMessage());
        }
    }
}